...
const CartDrawer = ({ isOpen, onClose, onBrowse }) => {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div 
        className="fixed top-0 right-0 h-full w-[450px] max-w-[100vw] bg-[#f2eadd] z-[1000] flex flex-col border-l-[4px] border-black shadow-2xl transform transition-transform duration-300 translate-x-0"
            ...
            <div className="space-y-6">
              {cartItems.map((item, idx) => (
                <div key={idx} className="bg-white border-[4px] border-black rounded-xl p-4 flex gap-4 relative">
                  <button onClick={(e) => { e.stopPropagation(); removeFromCart(item.title); }} className="absolute -top-3 -right-3 w-8 h-8 bg-white border-[3px] border-black rounded-full font-black flex items-center justify-center hover:bg-gray-200">×</button>
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <div className="flex items-center border-[3px] border-black rounded-lg overflow-hidden h-8 w-24">
                        <button onClick={(e) => { e.stopPropagation(); updateQuantity(item.title, -1); }} className="flex-1 font-black bg-[#f2eadd] hover:bg-gray-200">-</button>
